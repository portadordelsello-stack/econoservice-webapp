  ...
  const getFilteredList = () => {
    ...
    if (selectedDate) {
      list = list.filter(srv => {
        if (!srv.fechaIngreso) return false;
        try {
          const date = new Date(srv.fechaIngreso);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto flex-1 max-w-2xl">
            ...
            <div className="relative shrink-0 w-full sm:w-auto flex items-center">
              <div className="relative flex items-center bg-slate-50 dark:bg-gray-850 border border-slate-200/50 dark:border-gray-800 rounded-xl px-3 py-1.5 text-slate-900 dark:text-white focus-within:ring-2 focus-within:ring-indigo-500/20 transition-all w-full sm:w-auto select-none">
                <Calendar className="w-4 h-4 text-slate-400 shrink-0 mr-2" />
                <input
                  type="date"
                  value={selectedDate}
                  onChange={(e) => setSelectedDate(e.target.value)}
                  className="bg-transparent text-xs font-bold focus:outline-none border-none text-slate-700 dark:text-gray-200 w-[115px] cursor-pointer scheme-light dark:scheme-dark"
                />
                {selectedDate && (
                  <button
                    onClick={() => setSelectedDate("")}
                    className="ml-1.5 p-0.5 rounded-full hover:bg-slate-200 dark:hover:bg-gray-700 text-slate-400 hover:text-slate-600 dark:hover:text-gray-200 transition-colors shrink-0"
                    title="Limpiar fecha"
                  >
